...
def norm_hypervol(problem, hv, obj):
    ...
    from models import DTLZ1, DTLZ3, DTLZ5, DTLZ7
    max_hv = 150
    if problem.name == "dtlz1": max_hv = 122
    if problem.name == "dtlz3": max_hv = 122
    if problem.name == "dtlz5": max_hv = 135
    if problem.name == "dtlz7": max_hv = 116
    return hv / (max_hv ** exp) #usually 22


class NSGAII(object):
    def __init__(self, problem, num_of_generations, num_of_individuals, mutation_rate = "0.3", comparitor="bdom"):
    ...
    def __mutate(self, child, num_mutations):
        ...
        for gene in genes_to_mutate:
            child.features[gene] = child.features[
                                       gene] - self.mutation_rate / 2 + random.random() * self.mutation_rate
    ...
    def run(self):
        ...
        for i in range(self.num_of_generations):
            ...
            if self.hypervolume >= 0.90:
                #print "volbreak"
                break

        return self.hypervolume, self.population
