...
def populate(problem, size, init_val):
    population = []
    while(len(population) < size):
        if init_val == None:
            population.append(problem.generate_one())
        else:
            population.append(Point(init_val))
    return population
    ...
    while(True):
        ...
        for i in range(num_decisions):
            if (random.random() < crossover_rate) or (i == random_index):
                ...
                if i < 2:
                    mutated_decisions[i] = int(mutated_decisions[i])
...
def de(problem, init_val = None, pop_size = 100, gens = 100):
    population = populate(problem, pop_size, init_val)
    ...
    while gen < gens:
        ...
        print "de gen ", gen, "/", gens


    best_objs = [x.objectives for x in population]
    inital_objs = [x.objectives for x in initial_population]

    list = [cdom(x, y) for x, y  in zip(best_objs, inital_objs)]
    mean_cdom=  np.mean([cdom(x, y) for x, y  in zip(best_objs, inital_objs)])

    return gen, mean_cdom, population
